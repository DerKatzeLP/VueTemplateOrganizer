 ...
 */
function sortNodeAttributes(vueFileName, node) {
  ...
  if (node.children) {
    node.children.forEach((innerNode) => sortNodeAttributes(vueFileName, innerNode))
  }
}
 ...
 */
export function sortElementAttributes(vueFileName, template) {
  if (!config.sortVueFiles) return template

  const dom = parseTemplate(template)
  dom.forEach((innerNode) => sortNodeAttributes(vueFileName, innerNode))
...
async function replaceTemplateInVueSFC(filePath, vueFileName = 'xyz.vue') {
  try {
    ...
    const sortedTemplateContent = sortElementAttributes(vueFileName, templateInfo.content)

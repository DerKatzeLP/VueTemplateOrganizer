...
function logConfigError(configType, fileName) {
  ...
  console.error(
    `# Please add a valid ${fileName}${" ".repeat(Math.max(0, 37 - fileName.length))}#`,
  )
  console.log(
...
function sortNodeAttributes(vueFileName, node) {
  ...
  if (node.children) {
    node.children.forEach((innerNode) =>
      sortNodeAttributes(vueFileName, innerNode),
    )
  }
}
...
async function replaceTemplateInVueSFC(filePath, vueFileName = "xyz.vue") {
  try {
    ...
    const sortedTemplateContent = sortElementAttributes(
      vueFileName,

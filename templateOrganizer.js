...
const elementAttributeSorting = await loadSortingSettings()
const config = await loadConfigSettings()
...
function logConfigError(configType, fileName) {
    ...
    console.error(`# Could not load ${configType} file.${' '.repeat(Math.max(0, 35 - configType.length))}#`)
    console.error(`# Trying to use fallback ${fileName.split('.')[0]}.json${' '.repeat(Math.max(0, 28 - fileName.split('.')[0].length))}#`)
    console.error(`# Please add a valid ${fileName}${' '.repeat(Math.max(0, 37 - fileName.length))}#`)
    console.log(`# Add the file to your root folder: ./${fileName}${' '.repeat(Math.max(0, 20 - fileName.length))}#`)
...
async function loadSortingSettings() {
    return await loadConfigFile(SORTING_FILES, "sorting")
}
...
async function loadConfigSettings() {
    return await loadConfigFile(CONFIG_FILES, "config")
}
...
async function replaceTemplateInVueSFC(filePath, vueFileName = "xyz.vue") {
    try {
        ...
        if (!templateInfo) {
            console.error(`${COLORS.RED}The template content of ${vueFileName} was not found.${COLORS.RESET}`)
            return
        }

        const sortedTemplateContent = sortElementAttributes(templateInfo.content)

        htmlContent = htmlContent.substring(0, templateInfo.start) +
